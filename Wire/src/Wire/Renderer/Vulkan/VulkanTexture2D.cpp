...
namespace wire {
    ...
    namespace Utils {
        ...
        static void TransitionImageLayout(CommandList& commandList, VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
        {
            ...
            VulkanDevice* vk = (VulkanDevice*)commandList.getDevice();
    ...
    VulkanTexture2D::VulkanTexture2D(Device* device, const std::filesystem::path& path, std::string_view debugName)
        : m_Device(device), m_DebugName(debugName), m_UUID()
    {
        ...
        void* bufferData;
        vkMapMemory(vk->getDevice(), m_StagingBufferMemory, 0, imageSize, 0, &bufferData);
        memcpy(bufferData, data, imageSize);
        vkUnmapMemory(vk->getDevice(), m_StagingBufferMemory);
    ...
    VulkanTexture2D::VulkanTexture2D(Device* device, uint32_t* data, uint32_t width, uint32_t height, std::string_view debugName)
        : m_Device(device), m_DebugName(debugName), m_UUID()
    {
        ...
        if (data == nullptr)
        {
            ...
            std::memset(data, 0x00000000, width * height * sizeof(uint32_t));
            needsDelete = true;
        }
    ...
    void VulkanTexture2D::setData(const uint32_t *data, size_t size)
    {
        VulkanDevice* vk = (VulkanDevice*)m_Device;

        void* memory;
        vkMapMemory(vk->getDevice(), m_StagingBufferMemory, 0, size, 0, &memory);
