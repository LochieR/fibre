...
namespace wire {
    ...
    void CommandList::bindPipeline(const std::shared_ptr<GraphicsPipeline>& pipeline CMD_LIST_DEBUG_ARG)
    {
        CommandEntry entry;
        entry.Type = CommandType::BindPipeline;
        entry.Args = CommandEntry::BindPipelineArgs{ .IsGraphics = true, .Pipeline = pipeline };
    ...
    void CommandList::bindPipeline(const std::shared_ptr<ComputePipeline>& pipeline CMD_LIST_DEBUG_ARG)
    {
        CommandEntry entry;
        entry.Type = CommandType::BindPipeline;
        entry.Args = CommandEntry::BindPipelineArgs{ .IsGraphics = false, .Pipeline = pipeline };
    ...
    void CommandList::pushConstants(ShaderType shaderStage, const void* data, size_t size CMD_LIST_DEBUG_ARG, size_t offset)
    {
        WR_ASSERT(m_CurrentGraphicsPipeline || m_CurrentComputePipeline, "cannot push constants without binding a pipeline");
        WR_ASSERT(size <= 128, "push constant size must be <= 128");
    ...
    void CommandList::bindShaderResource(uint32_t set, const std::shared_ptr<ShaderResource>& resource CMD_LIST_DEBUG_ARG)
    {
        WR_ASSERT(m_CurrentGraphicsPipeline || m_CurrentComputePipeline, "cannot bind descriptor set without binding a pipeline");
    ...
    void CommandList::setViewport(const glm::vec2& position, const glm::vec2& size, float minDepth, float maxDepth CMD_LIST_DEBUG_ARG)
    {
        WR_ASSERT(m_CurrentGraphicsPipeline, "cannot set viewport without binding a pipeline");
    ...
    void CommandList::setScissor(const glm::vec2& min, const glm::vec2& max CMD_LIST_DEBUG_ARG)
    {
        WR_ASSERT(m_CurrentGraphicsPipeline, "cannot set scissor without binding a pipeline");
    ...
    void CommandList::setLineWidth(float lineWidth CMD_LIST_DEBUG_ARG)
    {
        WR_ASSERT(m_CurrentGraphicsPipeline, "cannot set line width without binding a pipeline");
    ...
    void CommandList::bindVertexBuffers(const std::vector<std::shared_ptr<Buffer>>& vertexBuffers CMD_LIST_DEBUG_ARG)
    {
        CommandEntry entry;
        entry.Type = CommandType::BindVertexBuffers;
        entry.Args = CommandEntry::BindVertexBuffersArgs{ .Buffers = vertexBuffers };
    ...
    void CommandList::bindIndexBuffer(const std::shared_ptr<Buffer>& indexBuffer CMD_LIST_DEBUG_ARG)
    {
        CommandEntry entry;
        entry.Type = CommandType::BindIndexBuffer;
        entry.Args = CommandEntry::BindIndexBufferArgs{ .Buffer = indexBuffer };
    ...
    void CommandList::draw(uint32_t vertexCount CMD_LIST_DEBUG_ARG, uint32_t vertexOffset)
    {
        WR_ASSERT(m_CurrentGraphicsPipeline, "cannot draw without binding graphics pipeline");
    ...
    void CommandList::drawIndexed(uint32_t indexCount CMD_LIST_DEBUG_ARG, uint32_t vertexOffset, uint32_t indexOffset)
    {
        WR_ASSERT(m_CurrentGraphicsPipeline, "cannot draw without binding graphics pipeline");
    ...
    void CommandList::dispatch(uint32_t groupCountX, uint32_t groupCountY, uint32_t groupCountZ CMD_LIST_DEBUG_ARG)
    {
        WR_ASSERT(m_CurrentComputePipeline, "cannot dispatch without binding compute pipeline");
    ...
    void CommandList::clearImage(const std::shared_ptr<Framebuffer>& framebuffer, const glm::vec4& color, AttachmentLayout currentLayout CMD_LIST_DEBUG_ARG, uint32_t baseMip, uint32_t numMips)
    {
        WR_ASSERT(m_CurrentScope.ScopeType == CommandScope::General, "cannot clear image inside a render pass");
    ...
    void CommandList::copyBuffer(const std::shared_ptr<Buffer>& srcBuffer, const std::shared_ptr<Buffer>& dstBuffer, size_t size CMD_LIST_DEBUG_ARG, size_t srcOffset, size_t dstOffset)
    {
        CommandEntry entry;
        entry.Type = CommandType::CopyBuffer;
        entry.Args = CommandEntry::CopyBufferArgs{ .SrcBuffer = srcBuffer, .DstBuffer = dstBuffer, .Size = size, .SrcOffset = srcOffset, .DstOffset = dstOffset };
    ...
    void CommandList::bufferMemoryBarrier(const std::shared_ptr<Buffer>& buffer, BarrierMask waitFor, BarrierMask access, PipelineStage waitStage, PipelineStage untilStage CMD_LIST_DEBUG_ARG)
    {
        CommandEntry entry;
        entry.Type = CommandType::BufferMemoryBarrier;
        entry.Args = CommandEntry::BufferMemoryBarrierArgs{ .WaitFor = waitFor, .Access = access, .WaitStage = waitStage, .UntilStage = untilStage, .Buffer = buffer };
    ...
    void CommandList::imageMemoryBarrier(const std::shared_ptr<Framebuffer>& framebuffer, AttachmentLayout oldLayout, AttachmentLayout newLayout CMD_LIST_DEBUG_ARG, uint32_t baseMip, uint32_t numMips)
    {
        CommandEntry entry;
        entry.Type = CommandType::ImageMemoryBarrier;
        entry.Args = CommandEntry::ImageMemoryBarrierArgs{ .Framebuffer = framebuffer, .OldUsage = oldLayout, .NewUsage = newLayout, .BaseMip = baseMip, .NumMips = numMips };
    ...
    void CommandList::submitNativeCommand(std::shared_ptr<CommandListNativeCommand> nativeCommand, std::type_index typeIndex CMD_LIST_DEBUG_ARG)
    {
        CommandEntry entry;
        entry.Type = CommandType::NativeCommand;
        entry.Args = CommandEntry::NativeCommandArgs{ .CommandType = typeIndex, .NativeCommand = nativeCommand };
