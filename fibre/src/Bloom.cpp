...
namespace fibre {
        ...
        : m_Device(device), m_Input(image), m_Width(width), m_Height(height)
    {
        ...
        framebufferDesc.Usage = wire::AttachmentUsage::TransferDst | wire::AttachmentUsage::TransferSrc | wire::AttachmentUsage::Storage;
        framebufferDesc.Layout = wire::AttachmentLayout::General;
        ...
        m_DownsampleResources->update(m_InputTexture, 0, 0);
        m_DownsampleResources->update(m_Sampler, 1, 0);

        m_DownsampleOutputResources.resize(m_MipCount);
        for (uint32_t i = 0; i < m_MipCount; i++)
        {
            m_DownsampleOutputResources[i] = device->createShaderResource(1, m_DownsampleLayout, "Bloom::m_DownsampleOutputResources");
            m_DownsampleOutputResources[i]->update(m_Intermediate, 0, 0, i);
        }
    }
    ...
    void Bloom::apply(wire::CommandList& commandList)
    {
        ...
        for (uint32_t i = 0; i < m_MipCount; i++)
        {
            ...
            commandList.bindShaderResource(1, m_DownsampleOutputResources[i]);
